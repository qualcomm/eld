    ...
    while (out != outEnd) {
      ...
      if (atTable.size() && m_AtTableIndex < atTable.size()) {
        ...
        if (curIsDebugSection ||
          ...
          // If the AT section falls within the bounds of the current address
          // and the size of the current section, try to place the AT section
          // inside the output section while traversing through the assignments.
