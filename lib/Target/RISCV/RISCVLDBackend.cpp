...
void RISCVLDBackend::relaxDeleteBytes(StringRef Name, RegionFragmentEx &Region,
                                      ...
                                      unsigned NumBytes, uint32_t NopBytes) {
  ...
  const ResolveInfo *Sym = Reloc->symInfo();
...
bool RISCVLDBackend::doRelaxationCall(Relocation *reloc) {
  ...
  if (canRelaxQcEJ) {
    ...
    relaxDeleteBytes(msg, *region, reloc, offset + 6, 2);
    region->attachDecision(reloc, ELF::riscv::internal::R_RISCV_QC_E_CALL_PLT,
                           qc_e_j, offset, 6);
...
bool RISCVLDBackend::doRelaxationQCCall(Relocation *reloc) {
  ...
  if (canCompress) {
    ...
    relaxDeleteBytes(msg, *region, reloc, offset + 2, 4);
    region->attachDecision(reloc, llvm::ELF::R_RISCV_RVC_JUMP, compressed,
                           offset, 2);
    return true;
  }
  ...
  if (isTailCall)
    msg = "RISCV_QC_E_J";
  relaxDeleteBytes(msg, *region, reloc, offset + 4, 2);
  region->attachDecision(reloc, llvm::ELF::R_RISCV_JAL, jal_instr, offset, 4);
...
bool RISCVLDBackend::doRelaxationLui(Relocation *reloc, Relocator::DWord G) {
  ...
  if (type == llvm::ELF::R_RISCV_HI20) {
    ...
    if (canRelaxZero)
      ...
      Msg = "RISCV_LUI_GP";
    if (!Msg.empty()) {
      relaxDeleteBytes(Msg, *region, reloc, offset, 4);
      if (canRelaxToGP) {
...
bool RISCVLDBackend::doRelaxationTLSDESC(Relocation &R, bool Relax) {
  ...
  auto attempt = [&]() -> bool {
    ...
    if (Relaxed)
      relaxDeleteBytes(RelaxType, *region, &R, offset, 4);
    else {
...
bool RISCVLDBackend::doRelaxationAlign(Relocation *pReloc) {
  ...
  uint32_t DeleteBytes = pReloc->addend() - NopBytesToAdd;
  bool DuplicateAlign =
      region->wouldOverlap(offset + NopBytesToAdd, DeleteBytes);
  if (!DuplicateAlign && DeleteBytes == 0) {
    for (Relocation *Other : region->getOwningSection()->getRelocations()) {
      if (Other != pReloc && Other->type() == llvm::ELF::R_RISCV_ALIGN &&
          Other->targetRef()->frag() == frag &&
          Other->targetRef()->offset() == offset) {
        DuplicateAlign = true;
        break;
      }
    }
  ...
  if (DuplicateAlign) {
    config().raise(Diag::error_riscv_duplicate_align_reloc)
        << region->getOwningSection()->name()
        << llvm::utohexstr(region->mapOffset(offset), true)
        << region->getOwningSection()
               ->getInputFile()
...
bool RISCVLDBackend::doRelaxationGOT(Relocation &Reloc) {
  ...
  if (SymInfo->isAbsolute() || SymInfo->isWeakUndef()) {
    ...
    if (Reloc.type() == llvm::ELF::R_RISCV_GOT_HI20) {
      ...
      relaxDeleteBytes(RelaxName, *region, &Reloc, Offset, 4);
    ...
    if (CanRelaxToCLi) {
      ...
      relaxDeleteBytes(RelaxName, *region, &Reloc, Offset + 2, 2);
...
bool RISCVLDBackend::verifyAndRollbackCallRelaxations() {
  ...
  for (CallRelaxRecord &Rec : m_CallRelaxRecords) {
    if (Rec.Dropped)
      continue;
    Relocation *Reloc = Rec.Reloc;
    RegionFragmentEx *Region = Rec.Region;
    ...
    bool Fits = (Rec.Kind == RelaxKind::CallToCJ) ? llvm::isInt<12>(X)
                                                  : llvm::isInt<21>(X);
    if (Fits)
      continue;
    ...
    if (m_Module.getPrinter()->isVerbose())
      config().raise(Diag::relax_call_rolled_back)
          << (Rec.Kind == RelaxKind::CallToCJ ? "RISCV_CALL_C" : "RISCV_CALL")
...
bool RISCVLDBackend::verifyAndRollbackGPRelaxations() {
  ...
  if (m_pGlobalPointer)
    GP = m_pGlobalPointer->value();
  for (GPRelaxRecord &Rec : m_GPRelaxRecords) {
    if (Rec.Dropped)
      continue;
    Relocation *HIReloc = Rec.HIReloc;
    ...
    // Nothing to restore: the edit was never applied.
    Rec.Region->dropEdit(Rec.EditReloc);
    if (m_Module.getPrinter()->isVerbose())
      config().raise(Diag::relax_gp_rolled_back)
          << (Rec.Kind == RelaxKind::PcGp ? "RISCV_PC_GP" : "RISCV_LUI_GP")
          << Rec.EditReloc->symInfo()->name()
          << Rec.Region->getOwningSection()->name()
          << llvm::utohexstr(
                 Rec.Region->mapOffset(Rec.EditReloc->targetRef()->offset()),
                 true)
          << Rec.Region->getOwningSection()
                 ->getInputFile()
                 ->getInput()
                 ->decoratedPath();
    Rec.Dropped = true;
...
void RISCVLDBackend::postRelax(bool &pFinished) {
  ...
  // both against each other before committing anything for real. Each round
  // that finds a mismatch permanently drops at least one candidate (Dropped
  // is sticky), so settling can never take more rounds than there are
  // candidates to drop; one extra round confirms the final state is clean.
  size_t MaxRounds = m_CallRelaxRecords.size() + m_GPRelaxRecords.size() + 1;
  if (static_cast<size_t>(m_PostRelaxRounds) >= MaxRounds) {
    config().raise(Diag::riscv_relaxation_error);
    return;
  }
  ++m_PostRelaxRounds;
  bool AnyDropped = verifyAndRollbackCallRelaxations();
