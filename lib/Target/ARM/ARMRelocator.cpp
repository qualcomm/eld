...
void ARMRelocator::handleScanForNonPreemptibleIFunc(Relocation &R,
  ...
  // Reserve a single IRELATIVE PLT entry per ifunc symbol.
...
void ARMRelocator::scanRelocation(Relocation &pReloc, eld::IRBuilder &pBuilder,
  ...
  if (Type == llvm::ELF::R_ARM_TARGET1) {
  ...
  // or global. Intercept here before the local/global dispatch.
...
Relocator::Result got_brel(Relocation &pReloc, ARMRelocator &pParent) {
  ...
  if (LLVM_UNLIKELY(isStaticIFunc)) {
    ARMGOT *G = pParent.getTarget().findEntryInGOT(rsym);
    if (G)
      GOT_S = G->getAddr(DiagEngine);
    else
      GOT_S = pParent.getTarget().findEntryInPLT(rsym)->getGOT()->getAddr(
          DiagEngine);
  } else {
    GOT_S = pParent.getTarget().findEntryInGOT(rsym)->getAddr(DiagEngine);
  }
  Relocator::DWord A = pReloc.target() + pReloc.addend();
  Relocator::Address GOT_ORG = pParent.getTarget().getGOTSymbolAddr();
  // Apply relocation.
