...
void ARMGNULDBackend::sortEXIDX() {
  ...
  for (auto &In : *O) {
    ...
    for (Fragment *F : S->getFragmentList()) {
      ...
      EXIDXFragment *EXIDX = static_cast<EXIDXFragment *>(F);
      ...
      // Relocations are created with input offsets; convert them to
      // piece-layout offsets after sorting/compaction.
      for (Relocation *R : Owning->getRelocations()) {
        if (!R || !R->targetRef() || R->targetRef()->isNull())
          continue;
        if (R->targetRef()->frag() != EXIDX)
    ...
    evaluateAssignments(S->getOutputSection());
  }
  ...
  for (auto &In : *O) {
    ...
    for (Fragment *F : S->getFragmentList()) {
      if (F->isNull())
        continue;
      if (!EXIDXFragSet.contains(F)) {
        // Non-EXIDX fragments (e.g. .comment) contribute their full size.
        TotalSize += F->size();
        continue;
      }
      // EXIDXFragment::size() returns post-GC/sort size.
      TotalSize += F->size();
      if (!F->size())
        continue;
      if (!FirstEXIDXFrag)
