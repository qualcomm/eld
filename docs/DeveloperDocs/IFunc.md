...
- Arithmetic relocations used for forming address
...
The below relocations are not currently supported by eld.
For IFunc symbols, they should be resolved to the PLT[IFuncSymbol]
as well.
...
- RISCV has no relocations specific to load / store relocations.
...
- RISCV does not have arithmetic-specific relocations that is
  typically used to form address.
