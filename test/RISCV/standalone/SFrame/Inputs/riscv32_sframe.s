...
.globl _start
_start:
    # Standard RISCV32 function prologue
    addi sp, sp, -8
    sw ra, 4(sp)
    sw fp, 0(sp)
    addi fp, sp, 8
    ...
    # Standard RISCV32 function epilogue
    lw fp, 0(sp)
    lw ra, 4(sp)
    addi sp, sp, 8
    ret

// RISCV32-specific SFrame section
.section .sframe,"a"
.align 4
    // SFrame Header
    ...
    .byte 7                // sfh_abi_arch (SFRAME_ABI_RISCV32_ENDIAN_LITTLE)
    .byte -8               // sfh_cfa_fixed_fp_offset (Frame pointer offset)
    .byte -4               // sfh_cfa_fixed_ra_offset (Return address offset)
    ...
    .long 20               // sfh_fre_len (Frame row entries length)
    .long 24               // sfh_fdeoff (Function descriptor offset)
    .long 44               // sfh_freoff (Frame row entries offset)

    // Function Descriptor Entry (FDE)
    .long _start           // sfde_func_start_address
    .long 20               // sfde_func_size
