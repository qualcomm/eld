...
# RUN: not %link %linkopts atomic_abi_A6C.o atomic_abi_A7.o -o /dev/null 2>&1 | FileCheck %s --check-prefix=ATOMIC_ABI_ERROR
# ATOMIC_ABI_ERROR: Error
...
# RUN: not %link %linkopts atomic_abi_A6C.o atomic_abi_invalid.o -o /dev/null 2>&1 | FileCheck %s --check-prefix=ATOMIC_ABI_INVALID
# ATOMIC_ABI_INVALID: Error

# RUN: %llvm-mc -filetype=obj -triple=riscv64  atomic_abi_A6S.s -o atomic_abi_A6S.o
# RUN: %link %linkopts -Wno-attribute-mix atomic_abi_A6S.o atomic_abi_A6C.o -o atomic_abi_A6C_A6S
# RUN: llvm-readobj -A atomic_abi_A6C_A6S | FileCheck %s --check-prefix=A6C_A6S

# RUN: %link %linkopts -Wno-attribute-mix atomic_abi_A6S.o atomic_abi_A7.o -o atomic_abi_A6S_A7
...
# RUN: %link %linkopts -Wno-attribute-mix atomic_abi_unknown.o atomic_abi_A6C.o -o atomic_abi_A6C_unknown
# RUN: llvm-readobj -A atomic_abi_A6C_unknown | FileCheck %s --check-prefixes=UNKNOWN_A6C

# RUN: %link %linkopts atomic_abi_unknown.o diff_stack_align.o -o atomic_abi_none_unknown
# RUN: llvm-readobj -A atomic_abi_none_unknown | FileCheck %s --check-prefixes=UNKNOWN_NONE

# RUN: %link %linkopts diff_stack_align.o atomic_abi_A6C.o -o atomic_abi_A6C_none
# RUN: llvm-readobj -A atomic_abi_A6C_none | FileCheck %s --check-prefixes=NONE_A6C

# RUN: %link %linkopts -Wno-attribute-mix atomic_abi_unknown.o atomic_abi_A6S.o -o atomic_abi_A6S_unknown
# RUN: llvm-readobj -A atomic_abi_A6S_unknown | FileCheck %s --check-prefix=UNKNOWN_A6S

# RUN: %link %linkopts -Wno-attribute-mix atomic_abi_unknown.o atomic_abi_A7.o -o atomic_abi_A7_unknown
# RUN: llvm-readobj -A atomic_abi_A7_unknown | FileCheck %s --check-prefix=UNKNOWN_A7

# RUN: %link %linkopts diff_stack_align.o atomic_abi_A7.o -o atomic_abi_A7_none
# RUN: llvm-readobj -A atomic_abi_A7_none | FileCheck %s --check-prefix=NONE_A7

# RUN: %link %linkopts -Wno-attribute-mix a.o atomic_abi_A6S.o -o atomic_abi_none_A6S
# RUN: llvm-readobj -A atomic_abi_none_A6S | FileCheck %s --check-prefix=NONE_A6S
...
#      NONE_A6S: BuildAttributes {
# NONE_A6S-NEXT:   FormatVersion: 0x41
# NONE_A6S-NEXT:   Section 1 {
# NONE_A6S-NEXT:     SectionLength: 105
# NONE_A6S-NEXT:     Vendor: riscv
# NONE_A6S-NEXT:     Tag: Tag_File (0x1)
# NONE_A6S-NEXT:     Size: 95
# NONE_A6S-NEXT:     FileAttributes {
# NONE_A6S-NEXT:       Attribute {
# NONE_A6S-NEXT:         Tag: 14
# NONE_A6S-NEXT:         Value: 2
# NONE_A6S-NEXT:         TagName: atomic_abi
# NONE_A6S-NEXT:         Description: Atomic ABI is 2
# NONE_A6S-NEXT:       }
# NONE_A6S-NEXT:       Attribute {
# NONE_A6S-NEXT:         Tag: 4
# NONE_A6S-NEXT:         Value: 16
# NONE_A6S-NEXT:         TagName: stack_align
# NONE_A6S-NEXT:         Description: Stack alignment is 16-bytes
# NONE_A6S-NEXT:       }
# NONE_A6S-NEXT:       Attribute {
# NONE_A6S-NEXT:         Tag: 5
# NONE_A6S-NEXT:         TagName: arch
# NONE_A6S-NEXT:         Value: rv64i2p1_m2p0_a2p1_f2p2_d2p2_c2p0_zicsr2p0_zmmul1p0_zaamo1p0_zalrsc1p0
# NONE_A6S-NEXT:       }
# NONE_A6S-NEXT:     }
# NONE_A6S-NEXT:   }
# NONE_A6S-NEXT: }
