...
# RUN: %not %link %linkopts -pie -z text %t.global -o %t.g.text.so 2>&1 \
# RUN:  | %filecheck %s --check-prefix=ERR
# RUN: %not %link %linkopts -pie -z text %t.local -o %t.l.text.so 2>&1 \
# RUN:  | %filecheck %s --check-prefix=ERR
# RUN: %not %link %linkopts -pie -z notext %t.global -o %t.g.notext.so 2>&1 \
# RUN:  | %filecheck %s --check-prefix=ERR
# RUN: %not %link %linkopts -pie -z notext %t.local -o %t.l.notext.so 2>&1 \
# RUN:  | %filecheck %s --check-prefix=ERR
